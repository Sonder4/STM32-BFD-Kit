...
Bootstrap 仅在 `.codex/bfd/active_profile.env` 缺失时必须立即停止。若报告的唯一 blocker 是缺少 `STM32_SVD`，可以继续进行原始 J-Link/GDB 调试、烧录、断点和 RAM/symbol 检查；这类缺口只会影响依赖 SVD 的寄存器语义视图与相关自动化。
...
On some recent J-Link GDB Server builds, `monitor reset halt` may print a parse error or leave the session less predictable than the split form above. Prefer the explicit two-command sequence unless you have already verified the combined form on the current probe/server revision.
...
monitor halt                         halt target after reset
