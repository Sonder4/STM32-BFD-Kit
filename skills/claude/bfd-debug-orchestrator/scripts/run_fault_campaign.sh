...
mkdir -p logs/debug/orchestrator logs/rtt logs/flash logs/build .claude/debug
...
RTT_MODE_USED=""
SUMMARY_PATH=".claude/debug/bfd-debug-orchestrator-campaign.md"
...
for s in "${SCENARIOS[@]}"; do
  run_logged "inject_s${s}" ./.claude/skills/bfd-debug-orchestrator/scripts/inject_fault_scenario.sh --scenario "$s" --elf "$ELF" --device "$DEVICE" --if "$IFACE" --speed "$SPEED"
  ...
  if [[ "$s" == "1" || "$s" == "2" ]]; then
    ...
    sleep 1
    run_logged "capture_s${s}" ./.claude/skills/bfd-debug-orchestrator/scripts/capture_hardfault_snapshot.sh --elf "$ELF" --scenario "$s" --device "$DEVICE" --if "$IFACE" --speed "$SPEED"
    ...
    if [[ -n "$HF_JSON" && -f "$HF_JSON" ]]; then
      ...
      run_logged "evolution_s${s}" ./.claude/skills/bfd-debug-orchestrator/scripts/update_error_evolution.py "$HF_JSON"
    fi
...
run_logged summary ./.claude/skills/bfd-debug-orchestrator/scripts/summarize_campaign.py --session-dir "$SESSION_DIR" --output "${SUMMARY_PATH}"
